...
type TaskStatus = "draft" | "assigned" | "running" | "blocked" | "completed" | "cancelled"
...
function Status({ status }: { status: TaskStatus }) { const labels: Record<TaskStatus, string> = { draft: "草稿", assigned: "待处理", running: "进行中", blocked: "受阻", completed: "已完成", cancelled: "已取消" }; const colors: Record<TaskStatus, string> = { draft: "bg-muted text-muted-foreground", assigned: "bg-blue-500/10 text-blue-700", running: "bg-amber-500/10 text-amber-700", blocked: "bg-red-500/10 text-red-700", completed: "bg-emerald-500/10 text-emerald-700", cancelled: "bg-muted text-muted-foreground" }; return <span className={`rounded px-2 py-0.5 text-xs font-medium ${colors[status]}`}>{labels[status]}</span> }
