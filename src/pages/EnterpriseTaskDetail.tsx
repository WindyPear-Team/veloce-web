...
export default function EnterpriseTaskDetail() {
  ...
  return <div className="space-y-5">
    ...
    <TaskDialog open={dialog === "participant"} title="添加任务参与人" pending={addParticipant.isPending} onClose={closeDialog} onSave={() => addParticipant.mutate()} disabled={!memberID}><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={memberID} onChange={(event) => setMemberID(event.target.value)}><option value="">选择员工</option>{availableMembers.map((item) => <option key={item.id} value={item.user_id}>{userName(item.user, item.user_id)}</option>)}</select></TaskDialog>
    <TaskDialog open={dialog === "department"} title="添加参与部门" pending={addDepartment.isPending} onClose={closeDialog} onSave={() => addDepartment.mutate()} disabled={!departmentID}><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={departmentID} onChange={(event) => setDepartmentID(event.target.value)}><option value="">选择部门</option>{availableDepartments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></TaskDialog>
    <TaskDialog open={dialog === "device"} title="下拨设备到任务池" pending={assignDevice.isPending} onClose={closeDialog} onSave={() => assignDevice.mutate()} disabled={!deviceID}><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={deviceID} onChange={(event) => setDeviceID(event.target.value)}><option value="">选择企业设备</option>{availableDevices.map((item) => <option key={item.id} value={item.id}>{item.name}{item.online ? "（在线）" : "（离线）"}</option>)}</select></TaskDialog>
    <TaskDialog open={dialog === "quota"} title="下拨额度到任务池" pending={allocateQuota.isPending} onClose={closeDialog} onSave={() => allocateQuota.mutate()} disabled={!parentQuotaID || !amount}><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={parentQuotaID} onChange={(event) => setParentQuotaID(event.target.value)}><option value="">选择来源额度账户</option>{(quotas.data?.accounts || []).filter((item) => item.id !== detail.data?.quota_account?.id).map((item) => <option key={item.id} value={item.id}>#{item.id} · {item.scope_type} · {item.scope_key}</option>)}</select><Input value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" placeholder="下拨额度" /></TaskDialog>
    <TaskDialog open={dialog === "resource"} title="分配资源到任务池" pending={assignResource.isPending} onClose={closeDialog} onSave={() => assignResource.mutate()} disabled={!resourceID}><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={resourceType} onChange={(event) => { setResourceType(event.target.value as "sessions" | "files"); setResourceID("") }}><option value="sessions">我的会话</option><option value="files">我的文件</option></select><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={resourceID} onChange={(event) => setResourceID(event.target.value)}><option value="">选择资源</option>{personalResources.map((item) => <option key={item.id} value={item.id}>{item.title || item.name || item.id}</option>)}</select><p className="text-xs text-muted-foreground">资源会复制到任务的内置池，参与人可在任务会话/文件夹中协作。</p></TaskDialog>
  </div>
}
