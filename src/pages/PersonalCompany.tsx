...
function Organization() {
  ...
  const agentsQuery = useQuery<AdvancedChatAgent[]>({ queryKey: ["advanced-chat-agents"], queryFn: async () => (await api.get("/advanced-chat/agents")).data })
  const devicesQuery = useQuery<ConnectorDevice[]>({ queryKey: ["advanced-chat-devices"], queryFn: async () => (await api.get("/advanced-chat/devices")).data })
  ...
  return <div className="space-y-5">
    ...
    <Dialog open={bindingEmployee !== null} onOpenChange={(open) => { if (!open && !bindRuntime.isPending) setBindingEmployee(null) }}><DialogContent><DialogHeader><DialogTitle>绑定员工运行时</DialogTitle><DialogDescription>{bindingEmployee?.name || ""} 将使用所选现有 Agent 的默认模型和既有能力配置。连接器操作仍走手动审批。</DialogDescription></DialogHeader><div className="grid gap-3"><select value={agentID} onChange={(event) => setAgentID(event.target.value)} className="h-10 rounded-md border bg-background px-3 text-sm"><option value="">选择已有 Agent</option>{(agentsQuery.data || []).filter((agent) => agent.default_model).map((agent) => <option key={agent.id} value={agent.id}>{agent.name} · {agent.default_model}</option>)}</select><select value={connectorID} onChange={(event) => setConnectorID(event.target.value)} className="h-10 rounded-md border bg-background px-3 text-sm"><option value="">不使用连接器</option>{(devicesQuery.data || []).filter((device) => device.status === "online").map((device) => <option key={device.id} value={device.id}>{device.name}</option>)}</select>{connectorID && <Input value={workspacePath} onChange={(event) => setWorkspacePath(event.target.value)} placeholder="连接器工作区绝对路径" />}</div><DialogFooter><Button disabled={!agentID || bindRuntime.isPending} onClick={() => bindRuntime.mutate()}>{bindRuntime.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "绑定"}</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
