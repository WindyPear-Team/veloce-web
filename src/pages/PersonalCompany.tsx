...
export default function PersonalCompany() {
  ...
  return <PersonalCompanyStudioContext.Provider value={groupID}><div className="mx-auto max-w-7xl space-y-5">
    ...
    {tab === "工作" && <WorkBoard workItems={workItems} onCreate={() => setIsWorkDialogOpen(true)} onCancel={(id) => cancelWork.mutate(id)} onQueue={(id) => queueWork.mutate(id)} queuePending={queueWork.isPending} onRun={(id) => runWork.mutate(id)} runPending={runWork.isPending} onViewInternal={setInternalWork} />}
...
function WorkBoard({ workItems, onCreate, onCancel, onQueue, queuePending, onRun, runPending, onViewInternal }: { workItems: WorkItem[]; onCreate: () => void; onCancel: (id: number) => void; onQueue: (id: number) => void; queuePending: boolean; onRun: (id: number) => void; runPending: boolean; onViewInternal: (item: WorkItem) => void }) {
