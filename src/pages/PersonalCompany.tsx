...
function StudioMemberDialog({ open, member, agents, saving, onOpenChange, onChange, onSave }: { open: boolean; member: StudioMember; agents: ChatAgent[]; saving: boolean; onOpenChange: (open: boolean) => void; onChange: (member: StudioMember) => void; onSave: () => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>编辑工作室成员</DialogTitle></DialogHeader><div className="grid gap-4"><label className="grid gap-2 text-sm font-medium">绑定 Agent<Select value={String((member.chat_agent_id || "") || "__shadcn_empty__")} onValueChange={(value) => { const agent = agents.find((item) => item.id === (value === "__shadcn_empty__" ? "" : value)); onChange({ ...member, chat_agent_id: (value === "__shadcn_empty__" ? "" : value), name: agent?.name || member.name }) }}><SelectTrigger className="h-10 rounded-2xl border bg-background px-3 text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="__shadcn_empty__">选择 Agent</SelectItem>{agents.map((agent) => <SelectItem key={agent.id} value={String(agent.id)}>{agent.name}</SelectItem>)}</SelectContent></Select></label><label className="grid gap-2 text-sm font-medium">成员名称<Input value={member.name} onChange={(event) => onChange({ ...member, name: event.target.value })} /></label><label className="grid gap-2 text-sm font-medium">职称<Select value={String((member.type) || "__shadcn_empty__")} onValueChange={(value) => onChange({ ...member, type: (value === "__shadcn_empty__" ? "" : value) as StudioMember["type"] })}><SelectTrigger className="h-10 rounded-2xl border bg-background px-3 text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="chief">总经理</SelectItem><SelectItem value="worker">员工</SelectItem><SelectItem value="critic">业务分析员</SelectItem><SelectItem value="reviewer">测试员</SelectItem><SelectItem value="checker">检查员</SelectItem></SelectContent></Select></label></div><DialogFooter><Button variant="ghost" onClick={() => onOpenChange(false)}>取消</Button><Button disabled={saving} onClick={onSave}>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}保存成员</Button></DialogFooter></DialogContent></Dialog>
}
