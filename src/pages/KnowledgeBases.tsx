...
export default function KnowledgeBases() {
  ...
  if (selectedBase) {
    ...
    const availableModels = catalog.find((channel) => channel.id === currentEmbeddingChannelID)?.models || []
    return (
      <div className="space-y-6">
        ...
        {!storageEnabled ? <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">{copy.storageDisabled}</CardContent></Card> : <>
          ...
          <Card>
            ...
            <CardContent className="grid gap-3 lg:grid-cols-[180px_minmax(0,1fr)_auto]"><label className="grid gap-2 text-sm"><span>{copy.embeddingChannel}</span><select className="h-10 rounded-md border bg-background px-3 text-sm" value={currentEmbeddingChannelID || ""} onChange={(event) => { setEmbeddingSettingsEdited(true); setEmbeddingChannelID(Number(event.target.value) || 0); setEmbeddingModel("") }}><option value="">{copy.autoChannel}</option>{catalog.map((channel) => <option key={channel.id} value={channel.id}>{channel.name}</option>)}</select></label><label className="grid gap-2 text-sm"><span>{copy.embeddingModel}</span>{availableModels.length > 0 ? <select className="h-10 rounded-md border bg-background px-3 text-sm" value={currentEmbeddingModel} onChange={(event) => { setEmbeddingSettingsEdited(true); setEmbeddingModel(event.target.value) }}><option value="">{copy.selectEmbeddingModel}</option>{availableModels.map((model) => <option key={model} value={model}>{model}</option>)}</select> : <Input value={currentEmbeddingModel} placeholder="text-embedding-3-small" onChange={(event) => { setEmbeddingSettingsEdited(true); setEmbeddingModel(event.target.value) }} />}</label><div className="flex items-end"><Button disabled={isVectorizing || !currentEmbeddingModel.trim() || documents.length === 0} onClick={() => vectorizeKnowledgeBase(currentEmbeddingModel, currentEmbeddingChannelID)}>{isVectorizing ? copy.vectorizing : selectedBase.vectorized ? copy.revectorize : copy.vectorize}</Button></div><p className="lg:col-span-3 text-xs text-muted-foreground">{copy.embeddingHint}</p></CardContent>
          </Card>
