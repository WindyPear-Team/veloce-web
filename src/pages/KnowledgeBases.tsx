...
interface PluginEmbeddingModel { id: string; model: string; name: string; description: string; dimensions: number }
...
export default function KnowledgeBases() {
  ...
  const { data: pluginEmbeddingModels = [] } = useQuery<PluginEmbeddingModel[]>({
    queryKey: ["plugin-embedding-models"],
    queryFn: async () => {
      const res = await api.get("/user/advanced-chat/embedding-models")
      return isRecord(res.data) && Array.isArray(res.data.models)
        ? res.data.models.filter((item): item is PluginEmbeddingModel => isRecord(item) && typeof item.id === "string" && typeof item.model === "string" && typeof item.name === "string").map((item) => ({ id: item.id, model: item.model, name: item.name, description: typeof item.description === "string" ? item.description : "", dimensions: Number(item.dimensions || 0) }))
        : []
    },
  })
  ...
  if (selectedBase) {
    ...
    const currentEmbeddingChannelID = embeddingSettingsEdited ? embeddingChannelID : selectedBase.embedding_user_channel_id || 0
		const availableModels = currentEmbeddingChannelID
			...
			: Array.from(new Set([...catalog.flatMap((channel) => channel.models), ...pluginEmbeddingModels.map((model) => model.model)]))
		...
		const embeddingModelLabel = (model: string) => pluginEmbeddingModels.find((item) => item.model === model)?.name || model
    return (
